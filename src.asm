...
jal	GetOneAndFour
add	$s7, $v0, $0		# Saving number one
add	$s6, $v1, $0		# Saving number four
...
jal	GetHundred
add	$s5, $v0, $0		# Saving number one hundred
...
add	$s2, $v0, $0		# Gets v0	// Gets Smallest Even
add	$s1, $v1, $0		# Gets v1	// Gets Smallest Odd

lw	$v0, 0($sp)		# Pops v0
lw	$v1, 4($sp)		# Pops v1
add	$sp, $sp, $s6		# Stack Pile + 4
add	$sp, $sp, $s6		# Stack Pile + 4

add	$s4, $v0, $0		# Gets v0(sp)	// Gets Greatest Even
add	$s3, $v1, $0		# Gets v1(sp)	// Gets Greatest Odd
...
Loop:
	beq	$t4, $s5, End_Loop	# If counter == 100 break
	...
	add	$t3, $t3, $s6		# Next Pos
	add	$t4, $t4, $s7		# Counter += 1
	j Loop
...
Count_Loop:
	...
	beq	$t2, $s5, End_Count_Loop	# If counter == 100 break
	lw 	$t1, 0($t0)
